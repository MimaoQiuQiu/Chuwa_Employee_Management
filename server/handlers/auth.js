...
exports.signin = async function (req, res, next) {
  try {
    ...
    const { id, username, role, name, ducoments, feedback, onboarding_status } =
      employee;
  ...
  try {
    ...
    if (foundToken.status !== "pending") {
      return res
        .status(401)
        .json({
          error: { message: `Your register token is ${foundToken.status}!` },
        }); 
    }}
