...
export default function EmployeeVisa() {
  ...
  const { employee, status, documents } = useSelector(
  ...
  useEffect(() => {
    if (status === "successed") {
      ...
      if (documents?.length) {
        ...
        // handle case for there are no documents
        setDetail({
          documents: [],
  ...
  const handleSubmit = async () => {
    try {
      if (uploadedfileList.length) {
        await dispatch(
          uploadDocumentAction({ id: employee.id, document: selectedFile }),
        );
  ...
  const handleDelete = async (text, document) => {
    await dispatch(
      deleteDocumentAction({ id: employee.id, documentId: document.id }),
    );
