...
export default function HrVisaDetail() {
  ...
  useEffect(() => {
    if (status === "successed" && selectedEmployee.id === employeeId) {
      ...
      let next_step = null;
      const last =
        selectedEmployee.documents[selectedEmployee.documents.length - 1];
      if (last?.document_status !== "approved") {
        next_step =
          visaProcess[Math.min(3, selectedEmployee.documents.length - 1)];
      ...
      {detail ? (
        <div className="center-wrapper">
          <div className="detail">
            <Typography.Title level={2}>{`Visa Files`}</Typography.Title>
            ...
            >
              <div className="contentBox">
                ...
                <Form.Item label="Work Authorization">
                  <Input
                    value={detail.work_authorization.title}
                    disabled={true}
                  />
                </Form.Item>
