...
const EmployeeForm = ({
  ...
  const handleSaveForm = async () => {
    try {
      ...
      await dispatch(
        updateEmployeeAction({ id: formData?.id, employee: finalData }),
      );
      if (!errMessage && Object.keys(selectedFile).length > 1)
        await dispatch(
          uploadDocumentAction({ id: formData?.id, document: selectedFile }),
        );
      if (personalInfo) {
        setIsDisable(() => true);
      }
  ...
  const pdfUrlTransfer = (document) => {
    const blob = new Blob([new Uint8Array(document.content.data)], {
      type: "application/pdf",
    });
